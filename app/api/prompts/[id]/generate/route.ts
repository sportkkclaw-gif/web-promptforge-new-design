...
// Legacy endpoint kept for compatibility; now returns prompt-text payload only.
export async function POST(request: NextRequest, { params }: Params) {
  try {
    ...
    try {
      await deductCredits(user.id, 1, `Prompt generation: ${prompt.title}`, 'generation', params.id);
    } catch (quotaErr: any) {
