...
// GET /api/search/templates — full-text search with filters (marketplace entry point)
// Alias for /api/search with same contract; supports q, category, engine, minPrice, maxPrice, sortBy, limit, offset
...
export async function GET(request: NextRequest) {
  ...
  if (!TextSchema.safeParse(q).success || q.trim().length > 200) {
    return error('Search query must be 200 characters or fewer', 400);
  }

  if (engine !== undefined) {
    if (!TextSchema.safeParse(engine).success || engine.trim() === '') {
      return error('engine cannot be an empty string', 400);
    }
    if (engine.trim().length > 50) {
      return error('engine must be 50 characters or fewer', 400);
    }
  }
  ...
  if (Number.isNaN(maxPrice)) return error('maxPrice must be a valid integer', 400);
  ...
  if (Number.isNaN(limit) || Number.isNaN(offset)) {
    return error('limit and offset must be valid integers', 400);
  }
  ...
  try {
    const result = await searchPrompts(q.trim(), {
      categorySlug,
      engine,
      minPrice,
      maxPrice,
    ...
    console.error('[/api/search/templates] internal error:', err);
